  ...
  <div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('trainingrecordsfull?md='.$filtermd) }}">{{ $pageTitle }}</a></li>
 	...
 	<div class="page-content-wrapper">
		...
		<ul class="parsley-error-list">
		 ...
		 {{ Form::open(array('url'=>'trainingrecordsfull/save/'.SiteHelpers::encryptID($row['id']).'?md='.$filtermd.$trackUri, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
<div class="col-md-12">
						<fieldset><legend> Training Records Full</legend>
			...
			<div style="clear:both"></div>	
			  ...
			  <div class="form-group">
				...
				<button type="button" onclick="location.href='{{ URL::to('trainingrecordsfull?md='.$masterdetail["filtermd"].$trackUri) }}' " id="submit" class="btn btn-success ">  {{ Lang::get('core.sb_cancel') }} </button>
				</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#user_id").jCombo("{{ URL::to('trainingrecordsfull/comboselect?filter=tb_users:id:id|first_name|last_name') }}",
		{  selected_value : '{{ $row["user_id"] }}' });
		
		$("#training_task_id").jCombo("{{ URL::to('trainingrecordsfull/comboselect?filter=training_tasks:id:task_name') }}",
		{  selected_value : '{{ $row["training_task_id"] }}' });
		
		$("#conf_completed_by").jCombo("{{ URL::to('trainingrecordsfull/comboselect?filter=tb_users:id:id|first_name|last_name') }}",
		{  selected_value : '{{ $row["conf_completed_by"] }}' });
		
		$("#created_by").jCombo("{{ URL::to('trainingrecordsfull/comboselect?filter=tb_users:id:id|first_name|last_name') }}",

...
<div class="page-content row">
    ...
    <div class="page-content-wrapper">
        <div class="toolbar-line ">
            @if($access['is_add'] ==1)
                <a href="{{ URL::to('docs/add?md='.$masterdetail["filtermd"].$trackUri) }}"
            ...
            @if($access['is_excel'] ==1)
                <a href="{{ URL::to('docs/download?md='.$masterdetail["filtermd"].$trackUri) }}"
                   class="tips btn btn-xs btn-default" title="{{ Lang::get('core.btn_download') }}">
                    <i class="fa fa-download"></i>&nbsp;{{ Lang::get('core.btn_download') }} </a>
            @endif
            @if(Session::get('gid') ==1)
                <a href="{{ URL::to('module/config/docs') }}" class="tips btn btn-xs btn-default"
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu  icons-left pull-right">
                                    ...
                                    @if($access['is_detail'] ==1)
                                        <li>
                                            <a href="{{ URL::to('docs/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"><i
                                                        class="fa  fa-search"></i> {{ Lang::get('core.btn_view') }}</a>
                                        </li>
                                    @endif
                                    @if($access['is_edit'] ==1)
                                        <li>
                                            <a href="{{ URL::to('docs/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"><i

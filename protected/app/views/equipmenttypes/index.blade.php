...
<div class="page-content row">
    ...
    <div class="page-content-wrapper">
        <div class="toolbar-line ">
            @if($access['is_add'] ==1)
                <a href="{{ URL::to('equipmenttypes/add?md='.$masterdetail["filtermd"].$trackUri) }}"
            ...
            @if($access['is_excel'] ==1)
                <a href="{{ URL::to('equipmenttypes/download?md='.$masterdetail["filtermd"].$trackUri) }}"
                   class="tips btn btn-xs btn-default" title="{{ Lang::get('core.btn_download') }}">
                    <i class="fa fa-download"></i>&nbsp;{{ Lang::get('core.btn_download') }} </a>
            @endif
            @if(Session::get('gid') ==1)
                <a href="{{ URL::to('module/config/equipmenttypes') }}" class="tips btn btn-xs btn-default"
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu  icons-left pull-right">
                                    ...
                                    @if($access['is_detail'] ==1)
                                        <li>
                                            <a href="{{ URL::to('equipmenttypes/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"><i
                                                        class="fa  fa-search"></i> {{ Lang::get('core.btn_view') }}</a>
                                        </li>
                                    @endif
                                    @if($row->id != FRIDGE and $row->id != FREEZER)
                                        @if($access['is_edit'] ==1)
                                            <li>
                                                <a href="{{ URL::to('equipmenttypes/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}"><i
        ...
        @include('footer')
            <div class="alert alert-info">
                <p><span class="glyphicon glyphicon-exclamation-sign"></span>
                    Note: You will not be able to edit certain reserved categories eg. Fridge (Monitored). These form part of the functionality of the system</p>
            </div>
    </div>
</div>
<script>

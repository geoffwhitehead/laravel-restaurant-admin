<div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('usedeposit?md='.$masterdetail["filtermd"]) }}">{{ $pageTitle }}</a></li>
 	...
 	<div class="page-content-wrapper">   
	   <div class="toolbar-line">
	   		<a href="{{ URL::to('usedeposit?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-xs btn-default" title="{{ Lang::get('core.btn_back') }}"><i class="icon-table"></i>&nbsp;{{ Lang::get('core.btn_back') }}</a>
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('usedeposit/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_edit') }}"><i class="icon-pencil3"></i>&nbsp;{{ Lang::get('core.btn_edit') }}</a>
			@endif  		   	  
		</div>
	...
	<table class="table table-striped table-bordered" >
		<tbody>	
					...
					<tr>
						<td width='30%' class='label-view text-right'>Purchased Sale Id</td>
						<td>{{ SiteHelpers::gridDisplayView($row->purchased_sale_id,'purchased_sale_id','1:sales:id:id|site_id|sale_date') }} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'>Used Sale Id</td>

  ...
  <div class="page-content row">
    ...
    <div class="page-header">
      ...
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('checks?md='.$filtermd) }}">{{ $pageTitle }}</a></li>
 	...
 	<div class="page-content-wrapper">
		...
		<ul class="parsley-error-list">
		 ...
		 {{ Form::open(array('url'=>'checks/save/'.SiteHelpers::encryptID($row['id']).'?md='.$filtermd.$trackUri, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
<div class="col-md-12">
						<fieldset><legend> Checks / Jobs</legend>
			...
			<div style="clear:both"></div>	
			  ...
			  <div class="form-group">
				...
				<button type="button" onclick="location.href='{{ URL::to('checks?md='.$masterdetail["filtermd"].$trackUri) }}' " id="submit" class="btn btn-success ">  {{ Lang::get('core.sb_cancel') }} </button>
				</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#check_category").jCombo("{{ URL::to('checks/comboselect?filter=check_categories:id:name') }}",
		{  selected_value : '{{ $row["check_category"] }}' });
		
		$("#check_frequency").jCombo("{{ URL::to('checks/comboselect?filter=check_time_periods:id:name') }}",
		{  selected_value : '{{ $row["check_frequency"] }}' });
		
		$("#site_id").jCombo("{{ URL::to('checks/comboselect?filter=sites:id:address_city') }}",
		{  selected_value : '{{ $row["site_id"] }}' });
		
		$("#department_id").jCombo("{{ URL::to('checks/comboselect?filter=departments:id:name') }}",

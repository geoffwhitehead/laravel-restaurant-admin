...
class PaymentmethodsController extends BaseController {
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','paymentmethods.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('payment_methods'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','paymentmethods.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','paymentmethods.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('payment_methods');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'paymentmethods/add/'.$id.'?md='.$md.$trackUri :  'paymentmethods?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('paymentmethods/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('paymentmethods?md='.Input::get('md'));
	}			

...
class MaintenancelogController extends BaseController {
	...
	public $module = 'maintenancelog';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Maintenancelog();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','maintenancelog.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','maintenancelog.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','maintenancelog.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'maintenancelog/add/'.$id.'?md='.$md.$trackUri :  'maintenancelog?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('maintenancelog/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('maintenancelog?md='.Input::get('md'));
	}			

<?php
class ServicecontactsController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'servicecontacts';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Servicecontacts();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','servicecontacts.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('service_contacts'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','servicecontacts.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','servicecontacts.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('service_contacts');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'servicecontacts/add/'.$id.'?md='.$md.$trackUri :  'servicecontacts?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('servicecontacts/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('servicecontacts?md='.Input::get('md'));
	}			

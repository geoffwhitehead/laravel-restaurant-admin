...
class InvoiceController extends BaseController {
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('company_invoices'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','invoice.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','invoice.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'invoice/add/'.$id.'?md='.$md.$trackUri :  'invoice?md='.$md.$trackUri );

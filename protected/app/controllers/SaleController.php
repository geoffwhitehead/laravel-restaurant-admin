...
class SaleController extends BaseController
{
    ...
    function getAdd($id = null)
    {
        ...
        $this->data['invoices'] = DB::select('SELECT SUM(amount) AS amt FROM invoices WHERE cash_taken = 1 and cash_taken_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
        $this->data['voucher_sale'] = DB::select('SELECT SUM(amount) AS amt FROM vouchers WHERE used = 1 and used_sale_id IS NULL and site_used = ' . Session::get("sid")  .' and active = 1');
        $this->data['voucher_count'] = DB::select('SELECT COUNT(amount) AS amt FROM vouchers WHERE used = 1 and used_sale_id IS NULL and site_used = ' . Session::get("sid")  .' and active = 1');
        $this->data['deposit_sale'] = DB::select('SELECT SUM(deposit_amount) AS amt FROM deposits WHERE used = 1 and used_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
        $this->data['deposit_count'] = DB::select('SELECT COUNT(deposit_amount) AS amt FROM deposits WHERE used = 1 and used_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
        $this->data['used_invoices'] = DB::select('SELECT id, invoice_date, amount FROM invoices WHERE cash_taken = 1 and cash_taken_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
        $this->data['used_vouchers'] = DB::select('SELECT voucher_ref, authorized_by, amount FROM vouchers WHERE used = 1 and used_sale_id IS NULL and site_used = ' . Session::get("sid") .' and active = 1');
        $this->data['used_deposits'] = DB::select('SELECT booking_date_time, booking_name, booking_phone, booking_covers, deposit_amount FROM deposits WHERE used = 1 and used_sale_id IS NULL and site_id = ' . Session::get("sid") .' and active = 1');
        $this->data['purchased_deposits'] = DB::select("select booking_date_time, booking_name, booking_phone, booking_covers, deposit_amount FROM deposits WHERE purchased_sale_id is null and site_id = ".Session::get('sid')." and active = 1");
        ...
        if ($validator->passes()) {
            try {
                ...
                if ($inputID == "") {
                    ...
                    DB::update("UPDATE vouchers SET used_sale_id = " . $ID . " WHERE used = 1 and used_sale_id IS NULL and site_used = " . Session::get("sid")  .' and active = 1');
                    // assign deposits to sale here
                    DB::update('UPDATE deposits SET used_sale_id = ' . $ID . ' WHERE used = 1 and used_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
                    // assign cash invoices to sale here
                    DB::update('UPDATE invoices SET cash_taken_sale_id = ' . $ID . ' WHERE cash_taken = 1 and cash_taken_sale_id IS NULL and site_id = ' . Session::get("sid")  .' and active = 1');
                    //update sale id to purchased deposits
                    DB::update('update deposits set purchased_sale_id = '.$ID.' where purchased_sale_id is null and active = 1 and site_id = '.Session::get('sid') .' and active = 1');
                }

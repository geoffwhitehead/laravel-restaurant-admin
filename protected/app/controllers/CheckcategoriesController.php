<?php
class CheckcategoriesController extends BaseController {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'checkcategories';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Checkcategories();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','checkcategories.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('check_categories'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','checkcategories.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','checkcategories.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'checkcategories/add/'.$id.'?md='.$md.$trackUri :  'checkcategories?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('checkcategories/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('checkcategories?md='.Input::get('md'));
	}			

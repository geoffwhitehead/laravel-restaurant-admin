...
class SaleeditController extends BaseController
{

    protected $layout = "layouts.main";
    protected $data = array();
    public $module = 'saleedit';
    static $per_page = '10';
    ...
    public function getIndex()
    {
        ...
        $this->layout->nest('content', 'saleedit.index', $this->data)
    ...
    function getShow($id = null)
    {
        ...
        $this->data['id'] = $id;
        $this->data['access'] = $this->access;
        $this->layout->nest('content', 'saleedit.view', $this->data)->with('menus', $this->menus);
    }
    ...
    function postSave($id = 0)
    {
        ...
        if ($validator->passes()) {
            ...
            $data['sale_checked_by'] = Session::get('uid');
            ...
            $redirect = (!is_null(Input::get('apply')) ? 'saleedit/add/' . $id . '?md=' . $md . $trackUri : 'saleedit?md=' . $md . $trackUri);
            return Redirect::to($redirect)->with('message', SiteHelpers::alert('success', Lang::get('core.note_success')));
        } else {
            $md = str_replace(" ", "+", Input::get('md'));
            return Redirect::to('saleedit/add/' . $id . '?md=' . $md)->with('message', SiteHelpers::alert('error', Lang::get('core.note_error')))
